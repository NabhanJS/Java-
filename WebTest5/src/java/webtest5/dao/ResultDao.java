...
import webtest5.model.Result;
import webtest5.util.DBConnection;
...
public class ResultDao {
    ...
    public static int update(Result value) throws ClassNotFoundException{
        int status=0;
        String sql="update from result (name=?,email=?,address=?,password=?,district=?, where id=?)";
        try {
            PreparedStatement ps=db.getCon().prepareStatement(sql);
            ps.setString(1, value.getName());
            ps.setString(2, value.getEmail());
